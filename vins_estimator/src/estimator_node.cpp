...
void update()
{
    ...
    // 遗留的imu的buffer， 因为下面需要pop，所以copy一份
...
/**
 * @brief imu消息存入buffer，同时按照imu频率预测位姿并发送， 这样就可以提高里程计频率
 * @param[in] imu_msg
*/
void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
{
    ...
    // 关于线程锁和条件变量参考 https://www.jianshu.com/p/c1dfa1d40f53
    m_buf.lock();
...
void process()
{
    while (true)
    {
        ...
        lk.unlock(); // 数据buffer的锁解锁，回调可以继续塞数据
        m_estimator.lock(); // 进行后端求解， 不能和复位重启冲突
        // 给予范围的for循环， 这里局势遍历每组image和imu组合
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                else // 这是针对最后一个imu数据，需要做一个简单的线性插值
                {

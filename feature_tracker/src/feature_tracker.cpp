...
void FeatureTracker::setMask()
{
    ...
    for (auto &it : cnt_pts_id)
    {
        if (mask.at<uchar>(it.second.first) == 255)
        {
            ...
            // 把周围一个圆内全部设置为0，这个区域不允许别的特征点存在，避免特征点过于集中
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0) // 上一帧有特征点，就可以进行光流追踪了
    {
        ...
        vector<uchar> status;
        vector<float> err;
        // Step1 调用opencv接口进行LK光流追踪，通过输出状态位status剔除outlier
        // cur_img 上一帧； cur_pts 上一帧特征点
        // forw_img 当前帧； forw_pts 追踪的当前帧上的特征点
        // status：每个特征点追踪的状态
        cv::calcOpticalFlowPyrLK(cur_img, forw_img, cur_pts, forw_pts, status, err, cv::Size(21, 21), 3);

        for (int i = 0; i < int(forw_pts.size()); i++)
            // Step2 通过图像边界剔除outlier
            if (status[i] && !inBorder(forw_pts[i])) // 追踪状态好的同时检查在不在图像范围内
                status[i] = 0;
    ...
    // 被追踪到的是上一帧就存在的， 因此追踪次数+1
    for (auto &n : track_cnt)
        n++;

    if (PUB_THIS_FRAME)
    {
        // Step3 通过对极约束来剔除outlier
        rejectWithF();
    ...
    cur_img = forw_img; // 更新上一帧图像
    cur_pts = forw_pts; // 更新上一帧特征点
    undistortedPoints();
    prev_time = cur_time;
}
...
void FeatureTracker::rejectWithF()
{
    ...
    if (forw_pts.size() >= 8)
    {
        ...
        vector<uchar> status;
        // opencv接口计算本质矩阵，某种意义上也是一种对极约束的outlier剔除
